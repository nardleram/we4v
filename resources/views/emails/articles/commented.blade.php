...
<p>{{ $comment->user->username}} commented on <em>{{ $comment->commentable->title }}</em>, one of your articles.</p>

<p>{{ $comment->user->username}} said:</p>

<span style="color: #939090;">{!! $comment->body !!}</span>

<p>Visit your <a href="{{ route('article-show', $comment->commentable->slug) }}">article’s page</a> if you would like to respond.</p>
