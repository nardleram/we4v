<template>
    <div v-if="project.project_completed" class="grid gap-1 grid-cols-12 grid-rows-projectBox text-we4vBg bg-we4vGrey-800 p-2 w-full rounded shadow-md mb-1 tracking-tight">
        <div class="col-start-1 col-end-11 text-sm font-semibold text-we4vBlue content-center items-center cursor-pointer pt-1 max-h-16">
            ...
            <p class="text-we4vGreen-500 font-semibold uppercase text-xs">Project completed: {{ project.project_updated_at }}. <span class="text-we4vGrey-200 font-light normal-case">Planned end date: {{ project.project_end_date }}</span></p>
        </div>
        <div class="col-start-11 col-end-13 flex flex-row flex-nowrap justify-around p-0 content-center items-center max-h-16 text-we4vGrey-200">
            <div @click="$emit('activateShowCompletedProjectModal', project)">
                <i class="fas fa-eye h-5 cursor-pointer text-lg"></i>
            </div>
            <div>
...
export default {
    ...
    emits: [
        'activateShowCompletedProjectModal'
    ]
}
</script>