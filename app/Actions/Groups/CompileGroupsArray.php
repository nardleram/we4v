...
class CompileGroupsArray
{
    public function compileGroups($rawGroups, $is_admin) : array 
    {
        ...
        foreach ($rawGroups as $rawGroup) {
            ...
            if ( 
                ($rawGroup->membership_type === 'App\Models\Team') &&
                ($currentTeamId !== $rawGroup->team_id) &&
                ($loop > 0) ) {
                ++$teamCount;
                $teamMemberCount = 0;
            ...
            if ($rawGroup->group_id !== $currentGroupId) {
                ...
                $groups[$groupCount]['group_created_at'] = $rawGroup->group_created_at;
                $groups[$groupCount]['geog_area'] = $rawGroup->geog_area;

                $rawGroup->group_owner_username
                ? $groups[$groupCount]['group_owner_username'] = $rawGroup->group_owner_username
                : null;
            }
            ...
            if ($rawGroup->team_id && $rawGroup->membership_type === 'App\Models\Team') { // Got a team
                ...
                $groups[$groupCount]['teams'][$teamCount]['team_created_at'] = $rawGroup->team_created_at;
            ...
            if ($rawGroup->user_id) {
                if ($rawGroup->membership_type === 'App\Models\Group' && $rawGroup->member_user_id === $rawGroup->user_id) {
                    if (!in_array($rawGroup->member_user_id, $groupMembers)) {
                        ...
                        $groups[$groupCount]['groupMembers'][$groupMemberCount]['created_at'] = $rawGroup->created_at;
                        $groups[$groupCount]['groupMembers'][$groupMemberCount]['membership_id'] = $rawGroup->membership_id;
                        $groups[$groupCount]['groupMembers'][$groupMemberCount]['path'] = $rawGroup->path ? $rawGroup->path : 'images/nobody.png';
                        $groups[$groupCount]['groupMembers'][$groupMemberCount]['slug'] = $rawGroup->slug;
                ...
                if ($rawGroup->membership_type === 'App\Models\Team' && $rawGroup->member_user_id === $rawGroup->user_id) {
                    ...
                    $groups[$groupCount]['teams'][$teamCount]['teamMembers'][$teamMemberCount]['created_at'] = $rawGroup->created_at;
                    $groups[$groupCount]['teams'][$teamCount]['teamMembers'][$teamMemberCount]['membership_id'] = $rawGroup->membership_id;
                    $groups[$groupCount]['teams'][$teamCount]['teamMembers'][$teamMemberCount]['slug'] = $rawGroup->slug;
            ...
            if ($rawGroup->team_id && $rawGroup->membership_type === 'App\\Models\\Team' && $loop > 0) {
                $currentTeamId = $rawGroup->team_id;
            }
